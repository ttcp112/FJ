...
{
    [NuAuth]
    public class NoSaleDetailReportController : BaseController
    {

        private static Logger _logger = LogManager.GetCurrentClassLogger();
        // GET: NoSaleDetailReport
        ...
        public ActionResult Report(BaseReportModel model)
        {
            try
            {
                ...
                NoSaleDetailReportFactory noSaleDetailfactory = new NoSaleDetailReportFactory();
                model.FromDate = DateTimeHelper.SetFromDate(model.FromDate);
                model.ToDate = DateTimeHelper.SetToDate(model.ToDate);
                XLWorkbook wb = noSaleDetailfactory.ExportExcel_New(model, lstStore);
                ViewBag.wb = wb;
                string sheetName = string.Format("Report_No_Sale_Detail_{0}", DateTime.Now.ToString("MMddyyyy"));
                Response.Clear();
            ...
            {
                _logger.Error("No Sale Detail Report Error: " + ex);
