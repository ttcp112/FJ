    ...
    public class SBEmployeeController : HQController
    {
        ...
        public ActionResult Search(EmployeeViewModels model)
        {
            try
            {
                ...
                var data02 = new List<EmployeeModels>();
                foreach (var item in data)
                {
                    ...
                    if (item.ListEmpStore != null && item.ListEmpStore.Count > 1)
                    {
                        ...
                        {
                            EmployeeOnStoreModels empOnStore = item.ListEmpStore[i];
        ...
        public ActionResult Create(EmployeeModels model)
        {
            try
            {
                ...
                {
                    EmployeeOnStoreModels EmpOnStore = null;
                    foreach (var item in model.ListRoleWorkingTime)
                    {
                        if (item.RoleID == null)
                        {
                            ModelState.AddModelError("RoleWorkingTimeAlert_" + item.StoreID, CurrentUser.GetLanguageTextFromKey("Please choose role for store")
                                + " [" + item.StoreName + "]");
                            break;
                        }
                        ////Role - Store
                        EmpOnStore = new EmployeeOnStoreModels();
                        ...
                        for (int i = 0; i < item.ListWorkingTime.Count; i++)
                        {
                                ...
                                && !itemWorkingTime.To.Equals(CurrentUser.GetLanguageTextFromKey("OFF")))
                            {
                                ...
                                {
                                    ModelState.AddModelError("RoleWorkingTimeAlert_" + item.StoreID,
                                        CurrentUser.GetLanguageTextFromKey("From time must be less than To time for store") + " [" + item.StoreName + "]");
                                    break;
                                }
                            }
                            //WorkingTime
                ...
                bool result = _factory.InsertOrUpdateEmployee(model, ref msg, mec);
                if (result)
                {
        ...
        public EmployeeModels GetDetail(string id, string StoreId)
        {
            try
            {
                EmployeeModels model = new EmployeeModels();
        ...
        public EmployeeModels GetDetailView(string id, string StoreId)
        {
            try
            {
                EmployeeModels model = new EmployeeModels();
        ...
        {
            EmployeeModels model = GetDetailView(id, StoreId);
            return PartialView("_View", model);
        }

        public PartialViewResult Edit(string id, string StoreId)
        {
            EmployeeModels model = GetDetail(id, StoreId);
        ...
        public ActionResult Edit(EmployeeModels model)
        {
            try
            {
                ...
                string msg = "";
                string mec = CurrentUser.ListOrganizationId[0];
                var result = _factory.InsertOrUpdateEmployee(model, ref msg, mec);
                if (result)
                {
                    //Save Image on Server
                    if (!string.IsNullOrEmpty(model.ImageURL) && model.PictureByte != null)
        ...
        public PartialViewResult ChangePassword(string id, string StoreId)
        {
            EmployeeModels model = GetDetail(id, StoreId);
        ...
        public ActionResult ChangePassword(EmployeeModels model)
        {
            ...
            try
            {
                ...
                if (!CommonHelper.GetSHA512(model.NewPassword).Equals(CommonHelper.GetSHA512(model.ConfirmPassword)))
                {
        ...
        public ActionResult Delete(EmployeeModels model)
        {
            ...
            catch (Exception ex)
            {
                ...
                ModelState.AddModelError("Name", CurrentUser.GetLanguageTextFromKey(ex.Message));// "Have an error when you delete an Employee");
                Response.StatusCode = (int)HttpStatusCode.BadRequest;
                return PartialView("_Delete", model);
            }
        }
        ...
        {
            RoleWorkingTime model = new RoleWorkingTime();
            model.GetListRole(StoreID, CurrentUser.ListOrganizationId);
            model.OffSet = currentOffset;
            model.StoreName = StoreName;
            model.StoreID = StoreID;
           
            /*Get GetListBusinessHour*/
            var storeInfor = _StoreFactory.GetListStores(null, StoreID)[0];
            if (storeInfor != null)
            {
                if (storeInfor.ListBusinessHour != null)
                {
                    List<WorkingTimeModels> ListWorkingTime = new List<WorkingTimeModels>();
                    foreach (var item in storeInfor.ListBusinessHour)
                    {
                    ...
                    model.ListWorkingTime = ListWorkingTime;
                }
            }
            ...
            if (model.ListWorkingTime != null)
            {
                model.ListWorkingTime = model.ListWorkingTime.OrderBy(x => x.Day).ToList();
                for (int i = 0; i < model.ListWorkingTime.Count; i++)
                {
                    var item = model.ListWorkingTime[i];
                    item.WorkingTimeID = null;
                    if (item.IsOffline)
                    {
                    ...
                    {
                        item.From = CommonHelper.ConvertToLocalTime(item.FromTime).ToString("HH:mm");
                        item.To = CommonHelper.ConvertToLocalTime(item.ToTime).ToString("HH:mm");
                    }
        ...
        public ActionResult Export(CustomerModels model)
        {
            try
            {
                ...
                Response.AddHeader("content-disposition", String.Format(@"attachment;filename={0}.xlsx",
                    CommonHelper.GetExportFileName("Employee").Replace(" ", "_")));
